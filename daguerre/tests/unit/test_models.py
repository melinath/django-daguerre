...
class AreaTestCase(BaseTestCase):
	def test_delete_adjusted_images__save(self):
		...
		adjusted1 = AdjustedImage.objects.create(requested_adjustment='fit', **kwargs)
		adjusted2 = AdjustedImage.objects.create(requested_adjustment='crop', **kwargs)

		area.save()

		self.assertRaises(AdjustedImage.DoesNotExist, AdjustedImage.objects.get, pk=adjusted2.pk)
